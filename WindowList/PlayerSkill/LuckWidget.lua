...
--** 日  期:	2017-11-01
--** 版  本:	1.0
--** 描  述:	玩家技能窗口——生活技能——幸运界面
--** 应  用:  
--******************************************************************/

local LuckWidget = UIControl:new
{
	windowName 	= "LuckWidget",
	m_curLevel = 0, -- 当前技能等级
}


function LuckWidget:Attach(obj)
...
-- 更新窗口
function LuckWidget:UpdateWidget()
	local hero = GetHero() 
    if not hero then 
        return 
    end
	...
	local skillLevel = skillPart:GetLifeSkillLevel(emLuck)
	
	local curCfg = IGame.rktScheme:GetSchemeInfo(LIFESKILLS_CSV, emLuck, skillLevel)
	if not curCfg then
		return
	end
...
-- 添加按钮的点击行为
function LuckWidget:OnAddBangGongButtonClick()
	local hero = GetHero()
	if not hero then
		return
	end
...
-- 添加按钮的点击行为
function LuckWidget:OnAddYinBiButtonClick()
...
-- 升级按钮的点击行为
function LuckWidget:OnUpgradeButtonClick()
	IGame.LifeSkillClient:CheckLifeSkillAndUpgrade(emLuck, self.m_curLevel)
end

-- 事件绑定
function LuckWidget:SubscribeEvent()
...
-- 移除事件的绑定
function LuckWidget:UnSubscribeEvent()
...
-- 更新银币
function LuckWidget:UpdateYinBi()
...
-- 更新帮贡
function LuckWidget:RefreshRecordData(eventData)
